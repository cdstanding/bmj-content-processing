...
set LIST=
title Publishing PPR %1%
for %%A in (%*) do set LIST=%%A,!LIST!
echo Send to HWX as Post Production Resend - loading script...

call ant -e -buildfile Z:\build_UploadtoBMJCMS.xml -propertyfile Z:\build_UploadtoBMJCMS_live.properties -Dinputdirlist="%LIST%" -Dlaunchedby="Send_to_HW_Post-prod-resend" -Dppr=true -Dembargo.set=n -Dpublishtohwx=false
NET USE Z: /DELETE
pause