...
<xsl:stylesheet xmlns:px="http://xmlcalabash.com/ns/document-sequence"
    xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xlink="http://www.w3.org/1999/xlink"
    xmlns:java="http://www.java.com/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="java xs" version="2.0">
    ...
    <xsl:variable name="gff" select="doc($graphics-found-filepath)"/>
    <xsl:variable name="source-file" select="doc($source)"/>
        ...
        <html>
            ...
            <body>
                <table>
                    ...
                    <tbody>
                        <xsl:if test="$source-file//comment()[matches(., 'Warning:', 'i')]">
                            <xsl:call-template name="extyles-warning"/>
                        </xsl:if>
                        <xsl:if test="not($source-file//comment()[matches(., 'Warning:', 'i')])">
    ...
    <xsl:template name="extyles-warning">
        ...
        <tr>
            <td>
                ...
                <ul class="guide">
                    <li>Red text is the source of the error in the <a href="{$extyles-xml}"
                                ><b>XML</b></a>.</li>
                    ...
                    <li>Check eXtyles <a
                            href="https://docs.google.com/a/bmj.com/spreadsheets/d/1E1rL6wRZrwMOnUoT62r2cjpAkbN5nNbEpJiuSyE4PcU/edit?usp=sharing"
                                ><b>Common Issues sheet</b></a>
                    </li>
    ...
    <xsl:template name="schematron">
        ...
        <xsl:variable name="meta-file" select="doc($meta)"/>
        ...
        <tr>
            <td class="th"><a href="https://docs.google.com/document/d/1AFG4oYB57crabyfUcCM7DlyefMLluMTeffCA-hwtwEA/edit?usp=sharing"><b>Common styling issues</b></a></td>
        </tr>
        <tr>
            <td class="th">
                <b>Article</b>
        ...
        <tr>
            ...
            <td colspan="3">
                <span>
                    <a href="{$content-store}/preview/jats-preview/{$art-id}.html">HTML Preview</a>
                </span>
            </td>
        </tr>
        <tr>
            ...
            <tr>
                ...
                <td colspan="2">
                    <xsl:for-each
                        ...
                        <p class="warntext"><b>Location in XML:</b>&#x00A0;<xsl:value-of
                                select="@location"/></p>
                        <p class="warntext">
            ...
            <tr>
                ...
                <td colspan="2">
                    <xsl:for-each
                        ...
                        <p class="failtext"><b>Location in XML:</b>&#x00A0;<xsl:value-of
                                select="@location"/></p>
                        <p class="failtext">
                            <xsl:value-of select="svrl:text/text()"/>
                        </p>
        ...
        <xsl:if test="$schema-invalid = 'schema-invalid'">
            <tr>
                <td class="th">
                    <b>Stage 3 validation (BMJ NLM XML)</b>
                </td>
                <td class="fail">Fail</td>
                <td colspan="2">
                    <p>
                    ...
                    <p>Your NLM XML has failed validation after conversion from JATS XML!</p>
                    <p>Resolve any Stage 2 validation errors first and if it still fails Contact Chris Standing (cstanding@bmj.com) or Sean Harrop (sharrop@bmj.com).</p>
                    <xsl:if
                        test="./following-sibling::svrl:failed-assert | ./following-sibling::svrl:successful-report">
                        <hr/>
                    </xsl:if>
