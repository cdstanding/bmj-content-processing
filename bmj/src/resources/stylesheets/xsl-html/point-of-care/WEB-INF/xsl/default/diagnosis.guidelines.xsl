      ...
      <head>
        <title>
          <xsl:value-of select="$monographTitle" />
		          ...
		          - <xsl:call-template name="translate">
						<xsl:with-param name="messagekey">diagnosis.guidelines</xsl:with-param>
					</xsl:call-template>					
				 -       
        ...
        <div id="main-body">
          <div class="body-copy">
            <div id="guidelines">
              <h1>
					<xsl:call-template name="translate">
						<xsl:with-param name="messagekey">body.diagnosis.guidelines.diagnostic-guidelines</xsl:with-param>
					</xsl:call-template>
              </h1>
              <xsl:choose>
                <xsl:when test="./diagnosis-guidlines/region">
                  <xsl:call-template name="showRegionalGuideline">
                  <xsl:with-param name="regions" select=".//region"/>
                  </xsl:call-template>
                </xsl:when>
                <xsl:when test="./diagnosis-guidlines/non-region/guideline">
                ...
                <xsl:otherwise>
                  <p>
					<xsl:call-template name="translate">
						<xsl:with-param name="messagekey">body.diagnosis.guidelines.evidence-centre</xsl:with-param>
					</xsl:call-template>                  
                  </p>
                </xsl:otherwise>
              </xsl:choose>
