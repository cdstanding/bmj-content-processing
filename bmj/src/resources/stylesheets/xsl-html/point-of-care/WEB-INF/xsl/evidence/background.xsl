...
<xsl:stylesheet version="2.0"
  ...
  <!-- nasty, nasty hacks to get the correct formatting - the content uses <b> tags
      to indicate where a new prargraph should start - crazy stuff. 
      
      These templates copied from CE and adjusted to make them work.
      They override the main ones in oak-general-lib 
      -->
  <xsl:template match="oak:p">
    <!-- is the first child not a strong -->
    <xsl:if test="name(child::node()[1])!='b'">
      <!-- yes: open the p tag -->
  ...
  <xsl:template match="oak:b">
    <xsl:choose>
      <xsl:when test="not(contains(., ':'))">
        <!-- don't break if we don't contain a colon -->
                                <xsl:if test="not(preceding-sibling::node()[last()])">
                                        <!-- but do start a para if we are first -->
                                        <xsl:text disable-output-escaping="yes">&lt;p&gt;</xsl:text>
                                </xsl:if>
        <strong><xsl:apply-templates select="node()">
