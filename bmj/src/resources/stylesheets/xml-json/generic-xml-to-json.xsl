...
<xsl:stylesheet version="2.0"
  ...
  <xsl:function name="json:create-simple-node-member" as="node()">
    ...
    <json:member>
      ...
      <json:value><xsl:copy-of select="translate(replace($value, '^\s+|\s+$', ''), '&#xA;&#xD;&#x002028;&#x002029;', '')"/></json:value>
    </json:member>
  </xsl:function>
        ...
        <xsl:choose>
          <xsl:when test="(count($node/namespace::*) gt 0 and $use-namespaces) or count($node/@*[not(../@json:force-array) or count(.|../@json:force-array)=2]) gt 0">
            <json:object>
              ...
              <json:member>
                <json:name>$</json:name>
                <json:value><xsl:value-of select="translate(replace($node, '^\s+|\s+$', ''), '&#xA;&#xD;&#x002028;&#x002029;', '')"/></json:value>
              </json:member>
            </json:object>
          </xsl:when>
          <xsl:otherwise>
                    ...
                    <xsl:choose>
                      ...
                      <xsl:otherwise>
                        <xsl:variable name="local-name">
                          <xsl:value-of select="current-group()[1]/local-name()"></xsl:value-of> 
                        </xsl:variable>
                  
                        <xsl:variable name="previous-sibling-nodes" select=". | preceding-sibling::*"/>
                        
                        
                        <xsl:variable name="preceeding-sibling-count-same-name">
                          <xsl:value-of select="count(current-group()[1]/preceding-sibling::*[./local-name()=current-group()[1]/local-name()]/following-sibling::*[1][./local-name()!=current-group()[1]/local-name()])"/>
                        </xsl:variable>
                        
                        <xsl:choose>
                          
                          <xsl:when test="$preceeding-sibling-count-same-name>0">
                            <xsl:value-of select="concat($local-name,$preceeding-sibling-count-same-name)"></xsl:value-of>
                          </xsl:when>
                          <xsl:otherwise>
      ...
      <xsl:when test="$use-badgerfish">
        <json:object>
          <json:member>
            <json:name>$</json:name>
            <json:value>
              <xsl:value-of select="translate(replace($node, '^\s+|\s+$', ''), '&#xA;&#xD;&#x002028;&#x002029;', '')"/>
            </json:value>
          </json:member>
        </json:object>
      </xsl:when>
  ...
  <xsl:function name="json:create-string" as="node()">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="preceeding-sibling-count-same-name">
          <xsl:value-of select="count($node/preceding-sibling::*[./local-name()=$node/local-name()]/following-sibling::*[1][./local-name()!=$node/local-name()])"/>
        </xsl:variable>
        
     <!--   <xsl:variable name="preceeding-sibling-count-different-name">
          <xsl:value-of select="count($node/preceding-sibling::*[./local-name()!=$node/local-name()]/preceding-sibling::*[1][./local-name()=$node/local-name()])"/>
        </xsl:variable>-->
  
        <json:name>
          <xsl:choose>
          <!--  <xsl:when test="$preceeding-sibling-count-different-name>0 or $preceeding-sibling-count-same-name>0">
            <xsl:choose>
             <xsl:when test=" $node/local-name()='para' and $preceeding-sibling-count-different-name>0">
                <xsl:value-of select="concat($node/local-name(),  $preceeding-sibling-count-different-name)"/>    
              </xsl:when> -->
             
             <!-- <xsl:when test="$preceeding-sibling-count-same-name>0 ">
                <xsl:value-of select="concat($node/local-name(), $preceeding-sibling-count-same-name)"/>
              </xsl:when>-->
              
              <xsl:when test="$preceeding-sibling-count-same-name>0 and (not (ends-with($node/local-name(), '-link') or $node/local-name()='organism' or $node/local-name()='tx-options'))">
                <xsl:value-of select="concat($node/local-name(),  $preceeding-sibling-count-same-name)"/>    
              </xsl:when>
  ...
  <xsl:function name="json:create-attributes" as="node()*">
    ...
    <xsl:for-each select="$node/@*[not(../@json:force-array) or count(.|../@json:force-array)=2]">
      <json:member>
        ...
        <json:value>
          <xsl:choose>
          <xsl:when test = "$node/@*[name() = 'target']">
            <xsl:value-of select="replace($node/@target, '.xml', '.json')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="."/>  
          </xsl:otherwise>
