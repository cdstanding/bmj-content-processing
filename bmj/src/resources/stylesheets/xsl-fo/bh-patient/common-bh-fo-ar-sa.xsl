...
<!-- Specifically for Arabic text, originally created for BH import export 0011300 -->
<xsl:stylesheet 
	...
	<xsl:template name="footer_copyright">
		<xsl:element name="fo:block" use-attribute-sets="">
			...
			<xsl:text disable-output-escaping="yes">&#xA9;</xsl:text> 
				<!-- needed so that the flow here is left to right, there is a rt-lb over the entire document -->
	...
	<xsl:template name="declaration_text">
		...
		<xsl:element name="fo:block" use-attribute-sets="">
			...
			<!--  This information is aimed at a patient audience. This information however does not replace medical advice. 
			If you have a medical problem please see your doctor. Please see our full--> 
			<xsl:text>هذه المعلومات موجهة للمرضى. غير أنها لا تعوض الإرشادات الطبية. إذا كانت لديك مشكلة طبية يرجى مراجعة الطبيب. الرجاء الاطلاع على </xsl:text>
			
			 <xsl:element name="fo:basic-link">
				<xsl:attribute name="external-destination">
					<xsl:text>url('http://group.bmj.com/group/about/legal/terms/')</xsl:text>
				</xsl:attribute>
				<xsl:element name="fo:inline" use-attribute-sets="link">
				    
				    <!-- Conditions of use -->
					<xsl:text>شروط الاستعمال</xsl:text>
				</xsl:element>
			</xsl:element>
			    <xsl:text> كاملة</xsl:text>
		</xsl:element>			
	</xsl:template>
	...
	<xsl:template match="sec">
		...
		<xsl:element name="fo:block" use-attribute-sets="">	
			<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
			<xsl:apply-templates>
				<!--CJP <xsl:with-param name="sectionDepth" select="xs:string(xs:integer($sectionDepth) + 1)"/> -->
			</xsl:apply-templates>
		</xsl:element>
	</xsl:template>
	...
	<xsl:attribute-set name="font-8pt">
		<xsl:attribute name="font-size">8pt</xsl:attribute>
		<xsl:attribute name="font-family">TraditionalArabic</xsl:attribute>
	</xsl:attribute-set>			
