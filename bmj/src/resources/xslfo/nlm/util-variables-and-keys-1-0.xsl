...
<!-- used to pass publication date from external process to PDF creation -->
<!-- ============================================================= -->
     
     <xsl:param name="batchPubDate"/>
...
<xsl:param name="graphics-location-drive" select="'C:'"/>
<xsl:param name="graphics-location-path"  select="'../graphics'"/>
<xsl:param name="logo-location-path"  select="'Z:/dev/bmj/logos'"/>
...
<xsl:variable name="titlefont"   select="'FreeSans'"/>
<xsl:variable name="textfont"    select="'FreeSerif'"/>
<xsl:variable name="textsize"    select="9.3"/>         <!-- points -->
<xsl:variable name="textleading" select="11"/>         <!-- points -->

<xsl:variable name="textboxLMarg" select="1.25"/>      <!-- inches -->
<xsl:variable name="textboxRMarg" select="1.25"/>      <!-- inches -->

<xsl:variable name="mainindent" select="0"/>
<!-- picas (pc) -->

                   <!-- points -->
<xsl:variable name="leading-between-body-paras"      select="'3pt'"/>
<xsl:variable name="leading-between-wrapped-paras"    select="'3pt'"/>
...
<xsl:variable name="space-below-tables-figures"         select="'18pt'"/>
     
     <xsl:variable name="leading-around-list-blocks"   select="'1pt'"/>
...
<xsl:variable name="fnsize"      select="7.5"/>          <!-- points -->
<xsl:variable name="fnleading"   select="9"/>         <!-- points -->
...
<xsl:variable name="title-for-running-head">
  ...
  <xsl:for-each select="/article/front/article-meta/title-group">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
           <xsl:apply-templates select="//back/notes[@notes-type='citation-text']/p"
                             mode="running-head"/>
      </xsl:otherwise>
      </xsl:choose>
...
<!--Select colour for sections, RMR, CR, Analysis, Practice are blue. Rest are red-->
     <xsl:variable name="section-colour">
          
               <xsl:choose>
                    <xsl:when test="matches(/article/front/article-meta/article-categories/subj-group[@subj-group-type='heading']/subject,'Research|Clinical|Practice')">
                         <xsl:text>#3E1C76</xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
                         <xsl:text>#B80505</xsl:text>
                    </xsl:otherwise>
               </xsl:choose>
     ...
     <xsl:variable name="section-column-count">
          
          <xsl:choose>
               <xsl:when test="matches(/article/front/article-meta/article-categories/subj-group[@subj-group-type='heading']/subject,'Editorial|Observation|Obituar|Research News|Short Cuts|Endgames|Minerva')">
                    <xsl:text>2</xsl:text>
               </xsl:when>
               <xsl:otherwise>
                    <xsl:text>2</xsl:text>
