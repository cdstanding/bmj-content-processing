...
-->      
         <fo:inline >
                   <xsl:choose>
                        <xsl:when test="matches($object-fig-type,'theme|lead|equation|fixed','i')">
<!--                             do nothing as figs will be placed in line-->
                        </xsl:when>
                        <xsl:when test="matches(ancestor::article/front/article-meta/article-categories/subj-group[@subj-group-type='heading'],'Editorial|Observation|Obituar|Short Cuts|Research News|Endgames|Minerva|Views &amp; Reviews','i')">
<!--                             do nothing as figs will be placed in line-->
                        </xsl:when>
                        <xsl:otherwise>
<!--                             figs placed at end of document provide a hot linked arrow-->
...
<xsl:template match="xref[@ref-type='bibr']">
  ...
  <!--<fo:wrapper>
    ...
    <xsl:text>]</xsl:text>
     ...
     <xsl:choose>
<!--          hide intermediate xref numbers-->
          <xsl:when
...
<!--               <xsl:comment>cond 1 suppress</xsl:comment>-->
          </xsl:when>
<!--          insert non-breaking hyphen-->
          <xsl:when
               test="preceding-sibling::node()[1][@ref-type='bibr'] and following-sibling::node()[1][@ref-type='bibr']">
               <fo:inline baseline-shift="35%" font-size="55%" >
<!--                    <xsl:comment>cond 2 replace with hyphen</xsl:comment>-->
                    <xsl:text>&#x2011;</xsl:text>
                    </fo:inline>
          </xsl:when>
<!--          insert non-breaking space-->
          <xsl:when
               test="preceding-sibling::node()[2][@ref-type='bibr'] and (normalize-space(translate(preceding-sibling::node()[1],' ',''))='')">
               <fo:inline baseline-shift="35%" font-size="55%" >
<!--                    <xsl:comment>cond 3 replace with non-break-space</xsl:comment>-->
                    <xsl:text>&#xA0;</xsl:text>
                    <xsl:apply-templates/>
               </fo:inline>
          </xsl:when>
<!--    insert the xref-->
          <xsl:when
               ...
               <fo:inline baseline-shift="35%" font-size="55%" >
<!--                    <xsl:comment>cond 4 add ref</xsl:comment>-->
                    <xsl:apply-templates/>
               </fo:inline>
          </xsl:when>
          <xsl:when test="preceding-sibling::node()[1][@ref-type='bibr']">
               <fo:inline baseline-shift="35%" font-size="55%" >
<!--                    <xsl:comment>cond 5 add non-break-space before </xsl:comment>-->
                    <xsl:text>&#xA0;</xsl:text>
                    <xsl:apply-templates/>
               </fo:inline>
          </xsl:when>
