...
<!--  <xsl:param name="space-after"  select="$leading-around-display-blocks"/>-->
     <xsl:param name="space-after"  select="$space-below-tables-figures"/>
  <xsl:param name="float">
    <xsl:choose>
<!--      <xsl:when test="@position='float'">before</xsl:when>
-->         <xsl:when test="@fig-type='theme'">
              <xsl:text>left</xsl:text>
         </xsl:when>
         <xsl:when test="@fig-type='equation'">
              <xsl:text>left</xsl:text>
         </xsl:when>
         <xsl:when test="@fig-type='fixed'">
              <xsl:text>left</xsl:text>
         </xsl:when>
         <xsl:when test="@position='float'">none</xsl:when>
         <xsl:otherwise>none</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
     <xsl:comment>PROCESSING FIG TEMPLATE IN PUT-AT-END MODE</xsl:comment>
...
-->         <!-- was buggy: space-before.conditionality="force" -->
    <fo:block line-height="10pt">
      ...
      <fo:wrapper font-family="{$titlefont}"
        ...
        <!--
             START:SPH 2010-1-15 Comment this out to avoid autogeneration of label
        ...
        <xsl:apply-templates select="caption/title" mode="display"/>
          ...
          <xsl:choose>
              <xsl:when test="label">
                  <fo:list-block space-before="1pt" space-before.precedence="1"
                 space-after="0pt"    space-after.precedence="1"
                 margin-left="1pc"
                 provisional-distance-between-starts="0.5pc"
                 provisional-label-separation="0.5pc">
                        <fo:list-item>
                            <fo:list-item-label font-weight="1000" start-indent="0pc" end-indent="1.5pc">
                                <fo:block>
                                    <xsl:apply-templates select="label"/>
                                </fo:block>
                            </fo:list-item-label>
                            <fo:list-item-body start-indent="2pc">
    ...
    <xsl:apply-templates select="copyright-statement|attrib" mode="display"/>
      </fo:wrapper>
    </fo:block>
     ...
     <xsl:template match="fig[not(matches(ancestor::article/front/article-meta/article-categories/subj-group[@subj-group-type='heading'],'Editorial|Observation|Obituar|Short Cuts|Research News|Endgames|Minerva|Views &amp; Reviews')) or matches(./@fig-type,'theme|lead|fixed|equation')]"/>
     <xsl:template match="fig[matches(ancestor::article/front/article-meta/article-categories/subj-group[@subj-group-type='heading'],'Editorial|Observation|Obituar|Short Cuts|Research News|Endgames|Minerva|Views &amp; Reviews')  or matches(./@fig-type,'theme|lead|fixed|equation')]">
          <xsl:param name="object-gid" select="generate-id()"/>
          ...
          <xsl:param name="float">
               <xsl:choose>
                    ...
                    <xsl:when test="@position='float'">start</xsl:when>
                    <xsl:otherwise>none</xsl:otherwise>
               </xsl:choose>
          </xsl:param>
          <xsl:comment>PROCESSING FIG TEMPLATE NO MODE</xsl:comment>
          ...
          <fo:float float="{$float}">
               <fo:block
                    ...
                    >
                    <xsl:choose>
                         <xsl:when test="matches(./@fig-type,'theme|lead|fixed|equation')">
<!--                              <xsl:attribute name="margin-left" select="'5pt'"/>
...
-->                              <xsl:attribute name="margin-right" select="'5pt'"/>
                              <xsl:attribute name="margin-top" select="'3pt'"/>
...
-->                         </xsl:when>
                         <xsl:otherwise>
                              <xsl:attribute name="margin-right" select="'5pt'"/>
                              <xsl:attribute name="space-before" select="$space-before"/>
                              <xsl:attribute name="space-after" select="$space-after"/>
                              <xsl:attribute name="margin-bottom" select="'3pt'"/>
                    ...
                    <fo:block line-height="10pt">
                         <fo:wrapper font-family="{$titlefont}"
                              ...
                              <!-- the fig/copyright or attribution if any -->
                              <xsl:apply-templates select="copyright-statement|attrib" mode="display"/>
                         </fo:wrapper>
...
<xsl:template match="fig/copyright-statement" mode="display">
  <fo:block>
    <fo:wrapper font-family="{$textfont}"
     ...
     <xsl:template match="fig/attrib" mode="display">
...
<!-- suppress in no-mode because have been
     ...
     <xsl:template match="fig/caption | fig/copyright-statement"/>
