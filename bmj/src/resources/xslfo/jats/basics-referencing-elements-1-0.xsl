...
<xsl:template name="make-reference">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$my-ref-type='boxed-text'
      ...
      <xsl:text>on page </xsl:text>
      <fo:page-number-citation ref-id="{$my-target-id}"/>-->
	  <fo:basic-link color="#5586E9" internal-destination="{@rid}">
		<xsl:apply-templates/>
	  </fo:basic-link>
    ...
    <xsl:when test="$my-ref-type='fig'">    
      ...
      <!--
           START: SPH 2010-15-1
           Comment out  label generation and insertions-->
...
-->      
        ...
        <xsl:variable name="rid-fig-processed">
                <xsl:choose>
                    <xsl:when test="@rid[contains(.,' ')]">
                        <xsl:value-of select="@rid/substring-before(.,' ')"/>
                    </xsl:when>
                    <xsl:otherwise>
         ...
         <fo:inline >
             <fo:basic-link color="#5586E9" internal-destination="{$rid-fig-processed}">
                 <xsl:apply-templates/>
             </fo:basic-link>
         </fo:inline>
    ...
    <xsl:when test="$my-ref-type='table'">
      ...
      <!--<xsl:for-each select="key('element-by-id', $my-target-id)">
        ...
        <xsl:variable name="rid-table-processed">
                <xsl:choose>
                    <xsl:when test="@rid[contains(.,' ')]">
                        <xsl:value-of select="@rid/substring-before(.,' ')"/>
                    </xsl:when>
         ...
         <fo:inline >
              <fo:basic-link color="#5586E9" internal-destination="{$rid-table-processed}">
                   <!--<xsl:choose>
                        <xsl:when test="preceding::table-wrap[@id=$my-target-id]">
...
<xsl:template match="xref[@ref-type='bibr']">
  ...
  <fo:inline baseline-shift="35%" font-size="55%" >
     ...
     <xsl:choose>
<!--          hide this xref value if it is beyond 2nd place in a consecutive list and is followed by at least 1 member of the same consecutive list -->
          <xsl:when
               test="(./text() - preceding-sibling::node()[2][@ref-type='bibr']/text() = 2) and (./text() - preceding-sibling::node()[1][@ref-type='bibr']/text() =1) and (following-sibling::node()[1][@ref-type='bibr']/text() - text() =1)">
<!--               <xsl:comment>cond 1 suppress ref</xsl:comment>-->
          </xsl:when>
       <!--        output this xref value when it is the last in a list of consecutive list of three or more xrefs-->
       <xsl:when
         test="(./text() - preceding-sibling::node()[2][@ref-type='bibr']/text() = 2) and (./text() - preceding-sibling::node()[1][@ref-type='bibr']/text() =1)">
<!--           <xsl:comment>cond 2 output xref as it is last in consecutive list</xsl:comment>-->
           <xsl:apply-templates/>
       </xsl:when>
       <!--          output a hyphen when this is the 2nd item in a consecutive list of three or more -->
          <xsl:when
               test="(./text() - preceding-sibling::node()[1][@ref-type='bibr']/text() =1) and (following-sibling::node()[1][@ref-type='bibr']/text() - ./text() = 1)">
<!--<xsl:comment>cond 3 replace with hyphen - this xref is 2nd of a consecutive list of 3 or more</xsl:comment>-->
                    <xsl:text>&#x2011;</xsl:text>
          </xsl:when>
<!--          insert non-breaking space and  xref value if this xref follows another biblio xref-->
          <xsl:when
               test="preceding-sibling::node()[1][@ref-type='bibr']">
<!--<xsl:comment>cond 4 output non-break-space and xref value - this xref is preceded by a non-conscutive one</xsl:comment>-->
                    <xsl:text>&#xA0;</xsl:text>
                    <xsl:apply-templates/>
          </xsl:when>
<!--    otherwise insert the xref-->
          <xsl:otherwise>
<!--                     <xsl:comment>cond 6 add ref</xsl:comment>-->
                    <xsl:apply-templates/>
          </xsl:otherwise>
     </xsl:choose>
  </fo:inline>
     ...
     <xsl:template
          ...
          and following-sibling::node()[1][@ref-type='bibr']]"> </xsl:template>
